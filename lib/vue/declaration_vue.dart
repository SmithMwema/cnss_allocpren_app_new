...
import '../utilitaire/validateur.dart'; // IMPORTANT
...
class DeclarationVue extends GetView<DeclarationCtrl> {
          ...
          Step(
            ...
            content: Form(
              ...
              child: Column(children: [
                ...
                _buildTextFormField(controller.emailAssureCtrl, 'Email', keyboardType: TextInputType.emailAddress, validator: Validateur.validerEmail),
                _buildTextFormField(controller.telAssureCtrl, 'Téléphone', keyboardType: TextInputType.phone, validator: Validateur.validerTelephone),
          ...
          Step(
            ...
            content: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'Date prévue d\'accouchement *', border: OutlineInputBorder()),
                readOnly: true,
              ...
              Row(children: [
                ElevatedButton.icon(onPressed: controller.selectionnerFichier, icon: const Icon(Icons.attach_file), label: const Text('Choisir un fichier')),
                const SizedBox(width: 10),
                Expanded(child: Obx(() => Text(controller.nomFichierSelectionne.value, overflow: TextOverflow.ellipsis))),
              ]),
            ]),
  ...
  Widget _buildTextFormField(TextEditingController ctrl, String label, {bool isRequired = true, TextInputType keyboardType = TextInputType.text, String? Function(String?)? validator}) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0),
      child: TextFormField(
        controller: ctrl,
        decoration: InputDecoration(labelText: isRequired ? '$label *' : label, border: const OutlineInputBorder()),
        keyboardType: keyboardType,
        validator: validator ?? (isRequired ? (value) => value == null || value.isEmpty ? 'Ce champ est requis' : null : null),
      ),
    );
  }
}